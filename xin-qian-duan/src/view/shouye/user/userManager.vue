<template>
 ...
 <!--=====================My练习案例===================================================-->
     ...
     <el-dialog
       ...
       <!--此组件的子组件 编辑2页面-->
        <userupdateinfo ref="userinfoVip"></userupdateinfo>d
...
<script>
    ...
    import userupdateinfo from '@/view/shouye/user/userUpdateInfo.vue'
    ...
    export default {
        ...
        components:{
          ...
          useradd,
          userupdate,
          userupdateinfo   //注册子组件
        },
        data(){
          return{
            myrowData:{},    //用于储存选中的数据,用于回显数据。他是一个对象
            dialogVisibleVip:false, //将初始化的弹出层默认为关闭状态
            rowData:{},
            roleValue:[],
            userId:"",
            loading8:false,
        ...
        methods:{
          digopened(){ //弹出层打开结束后，触发此事件
             //将页面选中的数据赋给子组件
             this.$refs.userinfoVip.toSetData(this.$data.myrowData) //父组件调用子组件，然后将父组件中选中的数据传给子组件的方法
          },
          useredit2(row){    //My演示案列       row选中的数据
              //点击编辑2按钮打开弹出层
              this.$data.dialogVisibleVip=true
              //给myrowData属性赋值，存储的是选中的数据
